...
public class NioServerDefinitionTest
{
	...
	public void testUpdate() throws Exception
	{
		...
		def3.setName("def3");
		def3.setPort(2000);
		def3.setInspectorFactory(new FakeNioObjectFactory<NioInspector>("def3Inspector"));
		def3.setSerializerFactory(new FakeNioObjectFactory<NioSerializer>("def3Serializer"));
		def3.setServiceFactories(Collections.<NioObjectFactory<NioService>>singletonList(new FakeNioObjectFactory<NioService>("def3Service")));

		//Setup our def2
		def4.setId("def4");
		def4.setName("def4");
		def4.setPort(1000);
		def4.setInspectorFactory(new FakeNioObjectFactory<NioInspector>("def4Inspector"));
		def4.setSerializerFactory(new FakeNioObjectFactory<NioSerializer>("def4Serializer"));
		def4.setServiceFactories(Collections.<NioObjectFactory<NioService>>singletonList(new FakeNioObjectFactory<NioService>("def4Service")));
		...
		//Our ID should not have changed
		assertEquals("The ID was changed when it should not have", "def3", def3.getId());

		//Check that the rest of the updates whent through apart from the port change
		assertEquals("The name was not updated correctly", def4.getName(), def3.getName());
		assertNotEquals("The port updated (should not have been)", def4.getPort(), def3.getPort());
		assertEquals("The Inspector was not updated correctly", def4.getInspectorFactory(), def3.getInspectorFactory());
		assertEquals("The Serializer was not updated correctly", def4.getSerializerFactory(), def3.getSerializerFactory());
		assertEquals("The Service Factories were not updated correctly", def4.getServiceFactories().get(0), def3.getServiceFactories().get(0));
	}
